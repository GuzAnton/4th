...
resource "digitalocean_firewall" "bastion" {
  ...
  inbound_rule {
    protocol         = "tcp"
    port_range       = "22"
    source_addresses = [var.MyIP, "194.44.97.147"]
  }
  outbound_rule {
    protocol              = "tcp"
    port_range            = "22"
...
resource "null_resource" "copy_ssl_certificates" {
  ...
  provisioner "local-exec" {
    command = "scp -o StrictHostKeyChecking=no /etc/letsencrypt/live/fourthestate.app-0001/*.pem root@${digitalocean_droplet.bastion.ipv4_address}:/etc/letsencrypt/live/test.fourthestate.app/"
  }
}
