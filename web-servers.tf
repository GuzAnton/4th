...
resource "null_resource" "copy_ssl_certificates" {
  ...
  provisioner "local-exec" {
    command = "scp -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa /root/etc/letsencrypt/live/fourthestate.app/*.pem root@${each.value}:/etc/letsencrypt/live/test.fourthestate.app/"
  }
}
